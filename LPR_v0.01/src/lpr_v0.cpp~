...
//	delta_x / delta_y
			...
			temp_slope = ((double)delta_x)/((double)(delta_y));

			for(iter = ivec.begin();iter != ivec.end();iter++)
			{
//	if in one line,num++,update the max length
				if(abs(iter->slope - temp_slope) < (double)0.5)
				{
					iter->num++;
...
dou_int res;
	...
	line(img_lines,res.v0,res.v1,Scalar(255,255,0),4);
...
*/
	...
	double angle = (double)atan(((double)(1.0))/res.slope);
	printf("angle is :%f\n",angle);
	Mat rot_mat = getRotationMatrix2D(center,-2.5,1.0);
